package com.ngo.gateway.filter;
...
import com.ngo.gateway.util.JwtUtil;
...
public class AuthenticationFilter extends AbstractGatewayFilterFactory<AuthenticationFilter.Config> {
	...
	private boolean isAuthorized(String role, String path, String method) {
		if ("ADMIN".equalsIgnoreCase(role)) {
			...
			return path.startsWith("/donations") || path.startsWith("/volunteers") || path.startsWith("/events");
		} 
		else if ("USER".equalsIgnoreCase(role)) {
			
			return ((path.startsWith("/donations") || path.startsWith("/volunteers") || path.startsWith("/events"))
					&& method.equalsIgnoreCase("GET"))    || 
					(path.startsWith("/donations") && method.equalsIgnoreCase("POST")) || 
					(path.startsWith("/volunteers") && (method.equalsIgnoreCase("POST") || method.equalsIgnoreCase("DELETE"))) ;             
		}
		return false;
	}
